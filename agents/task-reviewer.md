...
3. Audit the evidence — cheaply, without re-running:
   ...
   - Re-run the task's test command yourself **only** when that evidence is missing, inconsistent, or stale (code changed after the reported run) — a routine re-run of green tests is wasted effort. When you must: `bash ${CLAUDE_PLUGIN_ROOT}/skills/run-tests/scripts/run_unittests.sh --no-build <prefix>`.
