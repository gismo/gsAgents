...
- **Delegate lookups**: when a stack frame names a symbol you need context for, spawn `gismo:scout` (**haiku**, Agent tool) with one precise question ("where is `gsFoo::bar` defined", "signature of X") — several in one message when a trace raises several — instead of reading the library yourself. Use `gismo:indexer` (**sonnet**) when the question needs real exploration. Never spawn any other agent type; the diagnosis stays yours.
