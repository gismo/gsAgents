...
Locate sibling examples and APIs via `.claude/gismo-maps/library-map.md` and `.claude/gismo-maps/modules/<module>.md`. Still not enough? You may spawn the **haiku** explorer `gismo:indexer` (Agent tool) for cheap lookups — at most a couple per task. Never spawn any other agent type; if the spec stays ambiguous, report `RESULT: BLOCKED` instead of exploring further.
